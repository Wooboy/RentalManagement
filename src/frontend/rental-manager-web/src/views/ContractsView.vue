    ...
    <table class="table table-zebra">
      ...
      <tbody>
        <tr v-for="c in items" :key="c.id">
          <td>{{ c.contractNo }}</td><td>{{ c.tenant?.name }}</td><td>{{ c.propertyName }}</td><td>{{ c.monthlyRent }}</td>
...
const seed = ()=>({ id:0, contractNo:'', tenantId:0, startDateUtc:toDateInput(new Date().toISOString()), endDateUtc:toDateInput(new Date().toISOString()), monthlyRent:0, deposit:0, occupantCount:1, electricityRuleType:1, status:1 })
...
const edit = async(c:any)=>{ form.value={...c,startDateUtc:toDateInput(c.startDateUtc),endDateUtc:toDateInput(c.endDateUtc)}; selectedPropertyId.value=c.propertyUnitId||0; await loadRooms(); selectedRoomIds.value=c.propertyRoomIds||[]; error.value='' }
const validate = ()=>{ if(!form.value.contractNo.trim()) return '請輸入合約編號'; if(!form.value.tenantId) return '請選擇租客'; if(!selectedPropertyId.value) return '請選擇房源'; if(!selectedRoomIds.value.length) return '請至少選擇一間房間'; if(form.value.monthlyRent<0) return '月租不可小於0'; return '' }
const save = async()=>{ error.value=validate(); if(error.value) return; const payload={ contractNo:form.value.contractNo, tenantId:form.value.tenantId, propertyRoomIds:selectedRoomIds.value, startDateUtc:toIsoDate(form.value.startDateUtc), endDateUtc:toIsoDate(form.value.endDateUtc), monthlyRent:form.value.monthlyRent, deposit:form.value.deposit, occupantCount:form.value.occupantCount, electricityRuleType:form.value.electricityRuleType, status:form.value.status }; if(form.value.id) await api.put(`/contracts/${form.value.id}`,payload); else await api.post('/contracts',payload); reset(); await load() }
